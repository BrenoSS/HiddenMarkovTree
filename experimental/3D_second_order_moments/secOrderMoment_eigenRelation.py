...
class wavelet2order():
	...
	def secondOrderMatrixAnalysis(self, threshold):
		...
		if(self.points_finest_scale.shape[0] != 0):
			...
			relation_mask = (np.divide(w[:,:,:,0], w[:,:,:,2], out=np.zeros_like(w[:,:,:,2]), where=w[:,:,:,2]!=0) > threshold) #((w[:,:,:,0] / w[:,:,:,2]) > threshold)
