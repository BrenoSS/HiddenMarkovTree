...
class wavelet2order():
	...
	def secondOrderMatrixAnalysis(self, tmax):
		...
		if(self.points_finest_scale.shape[0] != 0):
			...
			out = np.zeros((secondOrderMatrix.shape[0], secondOrderMatrix.shape[1], secondOrderMatrix.shape[2]))

			# out[det <= 0] = 0
			# out[trace * det <= 0] = 0

			out[self.points_finest_scale[:,0], self.points_finest_scale[:,1], self.points_finest_scale[:,2]] = 1
